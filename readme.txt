...
On the other hand, giving links the same order number will apply whatever sort option you provid with `orderby`. For instance, leaving all links with an order number of zero will have no affect on ordering. This lets you create groups by giving sets of links the same order number. You can then sort within those groups with `orderyby` parameter.
